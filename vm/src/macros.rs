...
macro_rules! extend_module {
    ( $vm:expr, $module:expr, { $($name:expr => $value:expr),* $(,)? }) => {
        ...
        $(
            let val: PyObjectRef = ::std::convert::From::from($value);
            let ty = $crate::pyobject::TypeProtocol::class(&val);
            let val = if $crate::pyobject::IdProtocol::is(&ty, &$vm.ctx.types.builtin_function_or_method_type) {
                let m = $crate::obj::objfunction::PyMethod::new_nobind(module.clone(), val);
                $crate::pyobject::PyValue::into_ref(m, $vm).into_object()
            } else {
                val
            };
            $vm.set_attr(&module, $name, val).unwrap();
        )*
    }
}
